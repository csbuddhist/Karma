...
function SettingsPage({ state, update }: { state: ConsoleState; update: (state: ConsoleState) => void }) {
  return <div className="settings-grid"><Card className="full-span"><div className="setting-row"><div className="setting-icon green"><ShieldCheck /></div><div><strong>总保护开关</strong><p>修改后保存到保护服务；暂停与重新启用都需要已解锁的管理员会话。</p></div><Toggle checked={state.protectionEnabled} onChange={(value) => update({ ...state, protectionEnabled: value })} label="总保护" /></div></Card><Card><div className="card-heading"><div><h2>保护能力</h2><p>当前仓库实现进度</p></div></div><div className="capability-list"><div><Check />多显示器采集与本地 ONNX</div><div><Check />OCR 与关键词摘要</div><div><Check />Windows Service 与认证 IPC</div><div><Check />Agent watchdog 与健康心跳</div><div><Check />加密证据库与身份绑定处置端</div><div className="pending"><CircleAlert />风险融合与来源观察待接入</div></div></Card><Card><div className="card-heading"><div><h2>安全说明</h2><p>家庭家长控制边界</p></div></div><div className="security-note"><Lock /><p>控制台密码不能替代 Windows 账户隔离。若孩子拥有本机管理员权限，仍可能接管服务配置、文件权限或通过恢复环境绕过用户态保护。</p></div></Card></div>;
}
